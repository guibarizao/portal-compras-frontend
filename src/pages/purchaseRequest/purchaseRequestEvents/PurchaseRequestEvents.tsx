...
interface IPurchaseRequestEventsProps {
  title: string;
  purchaseRequestId: string;
}

const PurchaseRequestEvents: React.FC<IPurchaseRequestEventsProps> = ({
  purchaseRequestId,
  title,
}) => {
  ...
  const [purchaseRequestEvent, setPurchaseRequestEvents] = useState<IEvent[]>(
    []
  );
  ...
  const handleListAll = async () => {
    setLoading(true);
    await listPurchaseRequestEvents(purchaseRequestId)
      .then((response) => {
        setPurchaseRequestEvents(response);
      })
      .catch((error) => {
        ...
        <AccordionDetails>
          ...
          >
            ...
            >
              ...
              <TableBody>
                {purchaseRequestEvent.length > 0 &&
                  purchaseRequestEvent.map((purchaseRequestEvent) => {
                    return (
                      <TableRow
                        key={purchaseRequestEvent.id}
                        sx={{
                          "&:last-child td, &:last-child th": {
                            border: 0,
                          },
                      ...
                      >
                        ...
                        <CustomTableCell
                          size="small"
                          width={800}
                          title={purchaseRequestEvent.message}
                        >
                          {purchaseRequestEvent.message}
                        </CustomTableCell>
                        <CustomTableCell size="small" width={200}>
                          {purchaseRequestEvent.erpStatusDescription}
                        </CustomTableCell>
                        <CustomTableCell size="small" width={150}>
                          {moment(purchaseRequestEvent.date).format(
                            "DD/MM/YYYY HH:mm"
                          )}
                        </CustomTableCell>
                      </TableRow>
...
export default PurchaseRequestEvents;
