...
const ReportGenerate: React.FC<IReportsProps> = ({ title }) => {
  window.document.title = title;

  const { listAllReports, goToGenerateReport } = useReportService();
  ...
  async function handlePrintReport(reportId: string) {
    setLoading(true);

    await goToGenerateReport(reportId)
      .then((response) => {
        const loginWindow = window.open(response.urlLogin, "_blank");

        setTimeout(() => {
          loginWindow?.close();
          window.open(response.urlGenerate, "_blank");
        }, 800);
      })
      .catch((error) => {
        toastr.error(error?.message || "Contate a equipe de suporte");
                  ...
                  return (
                    ...
                    >
                      <CustomMenuItem
                        ...
                        handleToGenerateReport={handlePrintReport}
