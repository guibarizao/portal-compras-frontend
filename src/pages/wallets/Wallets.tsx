...
const Wallets: React.FC<IWalletsProps> = ({ title }) => {
  window.document.title = title;

  const { listAllWallets } = useWalletService();
  ...
  const handleToWallet = useCallback(
    (id: string | null) => {
      !!id ? navigate(`/wallet/${id}`) : navigate("/wallet");
    },
    [navigate]
  );
        ...
        >
                ...
                wallets.map((wallet) => {
                  return (
                    ...
                    >
                      ...
                      <CustomTableCell size="small" title="" width={100}>
          ...
          {wallets.length === 0 && (
            <CustomNoContentTableBody>
              <p>Nenhuma Ordem Estatística / Wallet encontrada</p>
            </CustomNoContentTableBody>
          )}
        </TableContainer>
