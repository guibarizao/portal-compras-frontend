...
interface IPurchaseRequestTypesProps {
  title: string;
}
...
const PurchaseRequestTypes: React.FC<IPurchaseRequestTypesProps> = ({
  title,
}) => {
  ...
  const handleToPurchaseRequestTypes = useCallback(
    (id: string | null) => {
      !!id
        ? navigate(`/purchase-request-type/${id}`)
        : navigate("/purchase-request-type");
    },
    [navigate]
  );
  ...
  return (
    <>
      <TitleContainer>
        <h1>Tipos de solicitação de compra</h1>
      </TitleContainer>
      <PageCard>
        <FilterButtonGroup>
          ...
          <Tooltip title="Adicionar tipo" placement="top">
            <IconButton
              onClick={() => handleToPurchaseRequestTypes(null)}
              size="small"
              color="primary"
              style={{ alignItems: "flex-end" }}
            >
                ...
                purchaseRequestTypes.map((purchaseRequestType) => {
                  return (
                    ...
                    >
                      <CustomMenuItem
                        id={
                          purchaseRequestType.id ? purchaseRequestType.id : ""
                        }
                        handleToEdit={handleToPurchaseRequestTypes}
                      />
                      ...
                      <TableCell
                        size="small"
                        width={200}
                        title={purchaseRequestType.description}
                      >
...
export default PurchaseRequestTypes;
