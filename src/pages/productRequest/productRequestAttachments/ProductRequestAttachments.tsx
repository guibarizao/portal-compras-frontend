...
import { ButtonCustom, Label } from "./ProductRequestAttachments.styles";
...
import { useProductRequestsAttachmentService } from "../../../services/useProductRequestsAttachmentsService";
...
interface IProductRequestsAttachmentsProps {
  title: string;
  productRequestId: string;
  productRequestDisableEdit: boolean;
}
...
const ProductRequestAttachments: React.FC<IProductRequestsAttachmentsProps> = ({
  productRequestId,
  productRequestDisableEdit,
  title,
}) => {
  ...
  const [productRequestAttachments, setProductRequestAttachments] = useState<
    IProductRequestsAttachment[]
  >([]);
  ...
  } = useProductRequestsAttachmentService();
  ...
  const handleListAll = async ({
    ...
    timeout.current = setTimeout(async () => {
      setLoading(true);
      await listAllProductRequestsAttachments(productRequestId, url)
        .then((response) => {
          if (response?.data?.length > 0) {
            setProductRequestAttachments(response.data);
            setTotalRows(response.totalRows);
            setLoading(false);
          } else {
            setProductRequestAttachments([]);
            setTotalRows(0);
            setPerPage(10);
            setCurrentPage(0);
            setLoading(false);
  ...
  const handleAttachmentChange = useCallback(
    async (e: ChangeEvent<HTMLInputElement>) => {
      if (e.target.files) {
        ...
        await uploadProductRequestsAttachment(productRequestId, data)
          .then(async () => {
    ...
    [navigate, productRequestId, filterField, filterPrecision, filterValue]
  );
  ...
  const handleAttachmentChangeUpdate = useCallback(
    async (
      ...
      if (e.target.files) {
        ...
        await updateProductRequestsAttachment(productRequestAttachmentId, data)
          .then(async () => {
            toastr.success("Anexo adicionado com sucesso");
            e.target.value = "";
  ...
  const handleDeleteProductRequestAttachment = useCallback(
    async (productRequestAttachmentId: string) => {
      const field = filterField;
      const value = filterValue;
      const precision = filterPrecision;

      setLoading(true);
      await deleteProductRequestsAttachment(productRequestAttachmentId)
        .then(() => {
      ...
      <Accordion elevation={0} defaultExpanded>
        ...
        <AccordionDetails>
          <FilterButtonGroup>
            ...
            <Tooltip
              title={`${productRequestDisableEdit ? "" : "Adicionar anexo"}`}
              placement="left"
            >
              <Label disabled={!!productRequestDisableEdit}>
                <AddOutlinedIcon />
                <input
                  type="file"
                  id="attachment"
                  ...
                  disabled={productRequestDisableEdit}
                ></input>
              </Label>
            </Tooltip>
          </FilterButtonGroup>
          ...
          >
            ...
            >
              ...
              <TableBody>
                {productRequestAttachments.length > 0 &&
                  productRequestAttachments.map((productRequestAttachment) => {
                    return (
                      <TableRow
                        key={productRequestAttachment.id}
                        sx={{
                          "&:last-child td, &:last-child th": {
                            border: 0,
                          },
                      ...
                      >
                        <CustomMenuItem
                          id={
                            productRequestAttachment.id
                              ? productRequestAttachment.id
                              : ""
                          }
                          handleSendAttachment={handleAttachmentChangeUpdate}
                          handleToDelete={handleDeleteProductRequestAttachment}
                          disableButton={productRequestDisableEdit}
                        />

                        <CustomTableCell size="small" width={150}>
                          {productRequestAttachment.file.length > 15 && (
                            <ButtonCustom
                              href={productRequestAttachment.file}
                              target="_blank"
                            >
                              <FiDownload size={16} />
                              Baixar Anexo
                        ...
                        <CustomTableCell
                          size="small"
                          width={350}
                          title={productRequestAttachment.filename}
                        >
                          {productRequestAttachment.filename}
                        </CustomTableCell>
                        <CustomTableCell size="small" width={130}>
                          {moment(productRequestAttachment.updated_at).format(
                            "DD/MM/YYYY HH:mm"
                          )}
                        </CustomTableCell>
                      </TableRow>
...
export default ProductRequestAttachments;
