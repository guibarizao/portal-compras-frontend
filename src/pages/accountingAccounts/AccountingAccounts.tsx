...
interface IAccountingAccountsProps {
  title: string;
}
...
const AccountingAccounts: React.FC<IAccountingAccountsProps> = ({ title }) => {
  window.document.title = title;

  const { listAllAccountingAccounts } = useAccountingAccountsService();
  ...
  const handleToAccountingAccount = useCallback(
    (id: string | null) => {
      !!id
        ? navigate(`/accounting-account/${id}`)
        : navigate("/accounting-account");
    },
    [navigate]
  );
  ...
  const handleListAll = async ({
    ...
    timeout.current = setTimeout(async () => {
      setLoading(true);
      await listAllAccountingAccounts(url)
        .then((response) => {
        ...
        >
              ...
              {accountingAccounts.length > 0 &&
                accountingAccounts.map((accountingAccount) => {
                  return (
                    ...
                    >
                      <CustomMenuItem
                        id={accountingAccount.id ? accountingAccount.id : ""}
                        handleToEdit={handleToAccountingAccount}
                        disableButton={true}
                      />

                      <CustomTableCell
                        size="small"
                        title={accountingAccount.code}
                        width={100}
                      >
                      ...
                      <CustomTableCell
                        size="small"
                        title={accountingAccount.description}
                        width={200}
                      >
                      ...
                      <CustomTableCell
                        size="small"
                        title={accountingAccount.classification}
                        width={200}
                      >
                        {accountingAccount.classification}
                      </CustomTableCell>
                      <CustomTableCell size="small" width={200}>
                        {accountingAccount.isActive ? "Sim" : "Não"}
                      </CustomTableCell>
                    </TableRow>
                  );
                })}
          ...
          {accountingAccounts.length === 0 && (
            <CustomNoContentTableBody>
              <p>Nenhuma conta contábil encontrada</p>
            </CustomNoContentTableBody>
          )}
        </TableContainer>
