...
const PaymentConditions: React.FC<IPaymentConditionsProps> = ({ title }) => {
  window.document.title = title;

  const { listAllPaymentConditions } = usePaymentConditionService();
  ...
  const handleToPaymentCondition = useCallback(
    (id: string | null) => {
      !!id
        ? navigate(`/paymentCondition/${id}`)
  ...
  const handleListAll = async ({
    ...
    timeout.current = setTimeout(async () => {
      setLoading(true);
      await listAllPaymentConditions(url)
        .then((response) => {
              ...
              {paymentConditions.length > 0 &&
                paymentConditions.map((paymentCondition) => {
                  return (
                    ...
                    >
                      <CustomMenuItem
                        id={paymentCondition.id ? paymentCondition.id : ""}
                        handleToEdit={handleToPaymentCondition}
                        disableButton={true}
                      />

                      <CustomTableCell
                        size="small"
                        title={paymentCondition.code}
                        width={100}
                      >
                      ...
                      <CustomTableCell size="small" width={200}>
                        {paymentCondition.isActive ? "Sim" : "Não"}
                      </CustomTableCell>
                    </TableRow>
                  );
                })}
